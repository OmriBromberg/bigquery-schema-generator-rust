// CLI definition for bq-schema-gen
//
// This file is included by both build.rs (via include!) and src/cli/mod.rs
// to ensure shell completions stay in sync with the actual CLI.
//
// IMPORTANT: Do not add `use` statements here - they are provided by the
// including file. Only use fully qualified paths or types from std/clap.
